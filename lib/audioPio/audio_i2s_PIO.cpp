...
void audio_i2s_init(uint data_pin, uint clock_pin_base, uint simple_rate) {
    // 1. 加载 PIO 程序
    offset = pio_add_program(TARGET_PIO, &audio_i2s_program);
    // 2. 获取默认配置
    ...
    float PIO_CLK_DIV = (float)clock_get_hz(clk_sys)/(float)(simple_rate *512);
    // 5. 硬编码时钟分频器
    ...
    gpio_set_slew_rate(clock_pin_base, GPIO_SLEW_RATE_FAST);
    gpio_set_function(clock_pin_base, GPIO_FUNC_PIO0);

    gpio_set_slew_rate(clock_pin_base + 1, GPIO_SLEW_RATE_FAST);
...
void audio_i2s_reset(uint sample_rate, uint channels) {
    ...
    pio_sm_restart(TARGET_PIO, TARGET_SM);
    float div = (float)clock_get_hz(clk_sys) / (float)(sample_rate * 512);
    pio_sm_set_clkdiv(TARGET_PIO, TARGET_SM, div);
        if(channels != 1){
        pio_sm_exec(TARGET_PIO, TARGET_SM, pio_encode_jmp(offset + audio_i2s_offset_stereo));
    }      
    else{
        pio_sm_exec(TARGET_PIO, TARGET_SM, pio_encode_jmp(offset + audio_i2s_offset_single));
    }    
    pio_sm_set_enabled(TARGET_PIO, TARGET_SM, true);
